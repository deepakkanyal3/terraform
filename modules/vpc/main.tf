locals {
  ...
  len_backend_subnets    = max(length(var.backend_subnets), length(var.backend_subnet_ipv6_prefixes))
...
#   #count = local.create_public_subnets && local.max_subnet_length > 0 ? 1 : 0
...
resource "aws_route_table" "public" {
  ...
  tags = merge(
    {
      "Name" = var.enable_network_firewall ? format(
        "${var.name}-${var.public_subnet_suffix}-%s",
        element(var.azs, count.index),) :  "${var.name}-${var.public_subnet_suffix}"
    },
    var.tags,
    var.public_route_table_tags,
    {
...
resource "aws_route_table_association" "public" {
  count = var.enable_network_firewall ? 1 : length(aws_subnet.public)

  subnet_id      = var.enable_network_firewall ? aws_subnet.public[0].id : aws_subnet.public[count.index].id
  ...
  tags = merge(
    {
      "Name" = format("%s-%s-%s",
                      ...
                      element(var.azs, count.index + 1))  // Adjusted index
    },
    var.tags,
    var.public_route_table_tags,
    {
...
resource "aws_route_table_association" "route2" {
  count = var.enable_network_firewall ? length(aws_subnet.public) : 0

  subnet_id      = aws_subnet.public[1].id
  route_table_id = aws_route_table.public2[0].id
}
  ...
  tags = merge(
    {
      "Name" = format("%s-%s-%s",
                      var.name,
                      var.public_subnet_suffix,
                      element(var.azs, count.index + 2))  // Adjusted index
    },
    var.tags,
    var.public_route_table_tags,
    {
...
resource "aws_route_table_association" "route3" {
  ...
  route_table_id = aws_route_table.public3[0].id
}
...
resource "aws_route" "public_internet_gateway" {
  count = local.create_public_subnets && (var.create_igw || var.firewall_endpoint_a != "") ? 1 : 0

  route_table_id         = aws_route_table.public[0].id
  destination_cidr_block = "0.0.0.0/0"

  
  vpc_endpoint_id = var.firewall_endpoint_a != "" ? var.firewall_endpoint_a : null
  gateway_id      = var.firewall_endpoint_a != "" ? null : aws_internet_gateway.this[0].id
...
resource "aws_route" "public_internet_gateway_2" {
  count = local.create_public_subnets && (var.create_igw || var.firewall_endpoint_b != "") && var.enable_network_firewall ? 1 : 0

  route_table_id         = var.enable_network_firewall ? aws_route_table.public2[0].id : null
  destination_cidr_block = "0.0.0.0/0"

  vpc_endpoint_id = var.firewall_endpoint_b != "" ? var.firewall_endpoint_b : null
  gateway_id      = var.firewall_endpoint_b != "" ? null : aws_internet_gateway.this[0].id
...
resource "aws_route" "public_internet_gateway_3" {
  count = local.create_public_subnets && (var.create_igw || var.firewall_endpoint_c != "") && var.enable_network_firewall ? 1 : 0

  route_table_id         = var.enable_network_firewall ? aws_route_table.public3[0].id : null
  destination_cidr_block = "0.0.0.0/0"

  vpc_endpoint_id = var.firewall_endpoint_c != "" ? var.firewall_endpoint_c : null
  gateway_id      = var.firewall_endpoint_c != "" ? null : aws_internet_gateway.this[0].id
...
resource "aws_route_table" "igw" {
  #count = local.create_firewall_subnets && local.max_subnet_length > 0 ? local.nat_gateway_count : 0
  count = var.enable_network_firewall ? 1 : 0
...
resource "aws_route" "firewall_endpoint_public1" {
  count = var.enable_network_firewall && var.firewall_endpoint_a != "" && local.create_firewall_subnets && var.create_igw ? 1 : 0

  route_table_id         = var.enable_network_firewall ? aws_route_table.igw[0].id : null
  destination_cidr_block = var.enable_network_firewall ? aws_subnet.public[0].cidr_block : null
  vpc_endpoint_id        = var.enable_network_firewall ? var.firewall_endpoint_a : null
...
resource "aws_route" "firewall_endpoint_public2" {
  count = var.enable_network_firewall && var.firewall_endpoint_b != "" && local.create_firewall_subnets && var.create_igw ? 1 : 0

  route_table_id         = var.enable_network_firewall ? aws_route_table.igw[0].id : null
  destination_cidr_block = var.enable_network_firewall ? aws_subnet.public[1].cidr_block : null
  vpc_endpoint_id        = var.enable_network_firewall ? var.firewall_endpoint_b : null
...
resource "aws_route" "firewall_endpoint_public3" {
  count = var.enable_network_firewall && var.firewall_endpoint_c != "" && local.create_firewall_subnets && var.create_igw ? 1 : 0

  route_table_id         = var.enable_network_firewall ? aws_route_table.igw[0].id : null
  destination_cidr_block = var.enable_network_firewall ? aws_subnet.public[2].cidr_block : null
  vpc_endpoint_id        = var.enable_network_firewall ? var.firewall_endpoint_c : null
...
# Uncomment this if custom_subnet is true
...
locals {
  create_backend_subnets     = local.create_vpc && local.len_backend_subnets > 0
  create_backend_route_table = local.create_backend_subnets && var.create_backend_subnet_route_table
}

resource "aws_subnet" "backend" {
  count = local.create_backend_subnets ? local.len_backend_subnets : 0

  assign_ipv6_address_on_creation                = var.enable_ipv6 && var.backend_subnet_ipv6_native ? true : var.backend_subnet_assign_ipv6_address_on_creation
  availability_zone                              = length(regexall("^[a-z]{2}-", element(var.azs, count.index))) > 0 ? element(var.azs, count.index) : null
  availability_zone_id                           = length(regexall("^[a-z]{2}-", element(var.azs, count.index))) == 0 ? element(var.azs, count.index) : null
  cidr_block                                     = var.backend_subnet_ipv6_native ? null : element(concat(var.backend_subnets, [""]), count.index)
  enable_dns64                                   = var.enable_ipv6 && var.backend_subnet_enable_dns64
  enable_resource_name_dns_aaaa_record_on_launch = var.enable_ipv6 && var.backend_subnet_enable_resource_name_dns_aaaa_record_on_launch
  enable_resource_name_dns_a_record_on_launch    = !var.backend_subnet_ipv6_native && var.backend_subnet_enable_resource_name_dns_a_record_on_launch
  ipv6_cidr_block                                = var.enable_ipv6 && length(var.backend_subnet_ipv6_prefixes) > 0 ? cidrsubnet(aws_vpc.this[0].ipv6_cidr_block, 8, var.backend_subnet_ipv6_prefixes[count.index]) : null
  ipv6_native                                    = var.enable_ipv6 && var.backend_subnet_ipv6_native
  private_dns_hostname_type_on_launch            = var.backend_subnet_private_dns_hostname_type_on_launch
  vpc_id                                         = local.vpc_id

  tags = merge(
    {
      Name = try(
        var.backend_subnet_names[count.index],
        format("${var.name}-${var.backend_subnet_suffix}-%s", element(var.azs, count.index), )
      )
    },
    var.tags,
...
resource "aws_db_subnet_group" "backend" {
  count = local.create_backend_subnets && var.create_backend_subnet_group ? 1 : 0

  name        = lower(coalesce(var.backend_subnet_group_name, var.name))
  description = "backend subnet group for ${var.name}"
...
resource "aws_route_table" "backend" {
  count = local.create_backend_route_table ? var.single_nat_gateway || var.create_backend_internet_gateway_route ? 1 : local.len_backend_subnets : 0

  vpc_id = local.vpc_id

  tags = merge(
    {
      "Name" = var.single_nat_gateway || var.create_backend_internet_gateway_route ? "${var.name}-${var.backend_subnet_suffix}" : format(
        "${var.name}-${var.backend_subnet_suffix}-%s",
        element(var.azs, count.index),
      )
    },
    var.tags,
    var.backend_route_table_tags,
...
resource "aws_route_table_association" "backend" {
  ...
  route_table_id = element(
    coalescelist(aws_route_table.backend[*].id, aws_route_table.private[*].id),
    var.create_backend_subnet_route_table ? var.single_nat_gateway || var.create_backend_internet_gateway_route ? 0 : count.index : count.index,
  )
}

resource "aws_route" "backend_internet_gateway" {
  count = local.create_backend_route_table && var.create_igw && var.create_backend_internet_gateway_route && !var.create_backend_nat_gateway_route ? 1 : 0
...
resource "aws_route" "backend_nat_gateway" {
  count = local.create_backend_route_table && !var.create_backend_internet_gateway_route && var.create_backend_nat_gateway_route && var.enable_nat_gateway ? var.single_nat_gateway ? 1 : local.len_backend_subnets : 0
...
resource "aws_route" "backend_ipv6_egress" {
  count = local.create_backend_route_table && var.create_egress_only_igw && var.enable_ipv6 && var.create_backend_internet_gateway_route ? 1 : 0
...
################################################################################
# backend Network ACLs
################################################################################

locals {
  create_backend_network_acl = local.create_backend_subnets && var.backend_dedicated_network_acl
}

resource "aws_network_acl" "backend" {
  count = local.create_backend_network_acl ? 1 : 0

  vpc_id     = local.vpc_id
  subnet_ids = aws_subnet.backend[*].id

  tags = merge(
    { "Name" = "${var.name}-${var.backend_subnet_suffix}" },
    var.tags,
    var.backend_acl_tags,
...
resource "aws_network_acl_rule" "backend_inbound" {
  count = local.create_backend_network_acl ? length(var.backend_inbound_acl_rules) : 0

  network_acl_id = aws_network_acl.backend[0].id

  egress          = false
  rule_number     = var.backend_inbound_acl_rules[count.index]["rule_number"]
  rule_action     = var.backend_inbound_acl_rules[count.index]["rule_action"]
  from_port       = lookup(var.backend_inbound_acl_rules[count.index], "from_port", null)
  to_port         = lookup(var.backend_inbound_acl_rules[count.index], "to_port", null)
  icmp_code       = lookup(var.backend_inbound_acl_rules[count.index], "icmp_code", null)
  icmp_type       = lookup(var.backend_inbound_acl_rules[count.index], "icmp_type", null)
  protocol        = var.backend_inbound_acl_rules[count.index]["protocol"]
  cidr_block      = lookup(var.backend_inbound_acl_rules[count.index], "cidr_block", null)
  ipv6_cidr_block = lookup(var.backend_inbound_acl_rules[count.index], "ipv6_cidr_block", null)
}

resource "aws_network_acl_rule" "backend_outbound" {
  count = local.create_backend_network_acl ? length(var.backend_outbound_acl_rules) : 0

  network_acl_id = aws_network_acl.backend[0].id

  egress          = true
  rule_number     = var.backend_outbound_acl_rules[count.index]["rule_number"]
  rule_action     = var.backend_outbound_acl_rules[count.index]["rule_action"]
  from_port       = lookup(var.backend_outbound_acl_rules[count.index], "from_port", null)
  to_port         = lookup(var.backend_outbound_acl_rules[count.index], "to_port", null)
  icmp_code       = lookup(var.backend_outbound_acl_rules[count.index], "icmp_code", null)
  icmp_type       = lookup(var.backend_outbound_acl_rules[count.index], "icmp_type", null)
  protocol        = var.backend_outbound_acl_rules[count.index]["protocol"]
  cidr_block      = lookup(var.backend_outbound_acl_rules[count.index], "cidr_block", null)
  ipv6_cidr_block = lookup(var.backend_outbound_acl_rules[count.index], "ipv6_cidr_block", null)
}
...
resource "aws_subnet" "firewall" {
  ...
  tags = merge(
    ...
    lookup(var.firewall_subnet_tags_per_az, element(var.azs, count.index), {}),
...
# resource "aws_route_table_association" "firewall" {
#   count = var.enable_network_firewall ? 1 : local.len_firewall_subnets

#   subnet_id      = var.enable_network_firewall ? aws_subnet.firewall[0].id : element(aws_subnet.firewall[*].id, count.index)
#   route_table_id = var.enable_network_firewall ? aws_route_table.firewall[0].id : element(aws_route_table.firewall[*].id, count.index)
# }
...
resource "aws_route" "firewall_internet_gateway" {
  count = local.create_firewall_subnets && var.create_igw ? 1 : 0
...
#   tags = merge(
#     {
#       "Name" = var.single_nat_gateway ? "${var.firewall_subnet_suffix}-2" : format(
#         "${var.name}-${var.firewall_subnet_suffix}-%s",
#         element(var.azs, count.index),
#       )
#     },
...
# resource "aws_route_table_association" "firewall_associate" {
#   count = var.use_custom_subnet_a ? 1 : local.len_firewall_subnets

#   subnet_id      = var.use_custom_subnet_a ? aws_subnet.firewall[1].id : element(aws_subnet.firewall[*].id, count.index)
#   route_table_id = aws_route_table.firewall_2[0].id
# }

# resource "aws_route" "firewall_internet_gateway_2" {
#   count = local.create_firewall_subnets && var.create_igw ? 1 : 0
