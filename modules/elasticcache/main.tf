...
resource "aws_elasticache_replication_group" "redis" {
  ...
  replication_group_id        = var.global_replication_group_id == null ? "${var.account}-${var.name}-${var.environment}" : "${var.account}-${var.name}-${var.environment}-replica"
  ...
  automatic_failover_enabled = var.automatic_failover_enabled && var.num_cache_clusters >= 1 ? true : false
...
resource "aws_elasticache_parameter_group" "redis" {
  name        = "${var.account}-${var.name}-${var.environment}-${random_id.redis_pg.hex}"
  family      = var.family
...
resource "aws_elasticache_subnet_group" "redis" {
  ...
  name        = var.global_replication_group_id == null ? "${var.account}-${var.name}-${var.environment}-redis-sg" : "${var.account}-${var.name}-${var.environment}-redis-sg-replica"
...
resource "aws_security_group" "redis" {
  name_prefix = "${var.account}-${var.name}-${var.environment}"
  vpc_id      = var.vpc_id
