...
locals {
  domain_name1 = "gromoinsure.in" # trimsuffix(data.aws_route53_zone.this.name, ".")
  subdomain1   = "statichi-stg"
}
...
module "cloudfront1" {
  ...
  aliases = ["${local.subdomain1}.${local.domain_name1}"]
  comment             = "stg-health-insurance"
  enabled             = true
  ...
  origin = {
    s3_one = {
      ...
      domain_name = module.s3_one1.s3_bucket_bucket_regional_domain_name
...
data "aws_iam_policy_document" "s3_policy1" {
  ...
  statement {
    ...
    resources = ["${module.s3_one1.s3_bucket_arn}/*"]

    principals {
      type        = "AWS"
      identifiers = module.cloudfront1.cloudfront_origin_access_identity_iam_arns
      # identifiers = ["*"]
    }
  }
  ...
  statement {
    ...
    condition {
      ...
      values   = [module.cloudfront1.cloudfront_distribution_arn]
    }
  }
}

resource "aws_s3_bucket_policy" "bucket_policy1" {
  bucket = module.s3_one1.s3_bucket_id
