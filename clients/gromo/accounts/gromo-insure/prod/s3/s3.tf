...
data "aws_iam_policy_document" "s3_policy7" {
  # Origin Access Identities
    statement {
    effect = "Allow"
    actions   = ["s3:PutObject"]
    resources = [
      "${module.s3-bucket2.s3_bucket_arn}/*.jpg",
      "${module.s3-bucket2.s3_bucket_arn}/*.png",
      "${module.s3-bucket2.s3_bucket_arn}/*.jpeg",
      "${module.s3-bucket2.s3_bucket_arn}/*.pdf",
      "${module.s3-bucket2.s3_bucket_arn}/*.doc",
      "${module.s3-bucket2.s3_bucket_arn}/*.docx"
    ]
...
resource "aws_s3_bucket_policy" "bucket_policy7" {
  bucket = module.s3-bucket2.s3_bucket_id
  policy = data.aws_iam_policy_document.s3_policy7.json
}
