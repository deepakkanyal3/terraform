...
locals {
  domain_name5 = "gromoinsure.co.in" # trimsuffix(data.aws_route53_zone.this.name, ".")
  subdomain5   = "sales"
}
...
module "cloudfront5" {
  ...
  aliases = ["${local.subdomain11}.${local.domain_name11}"]
  comment             = "Sales GroMo Insure Prod"
  enabled             = true
...
module "s3_one5" {
  ...
  consumer    = "User"
  producer    = "CDN"
  bucket      = "sales.gromoinsure.co.in"
  website = {
