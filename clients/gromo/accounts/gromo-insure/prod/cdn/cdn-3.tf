...
locals {
  domain_name3 = "gromoinsure.co.in" # trimsuffix(data.aws_route53_zone.this.name, ".")
  subdomain3   = "pos"
}
...
module "cloudfront3" {
  ...
  aliases = ["${local.subdomain9}.${local.domain_name9}"]
  comment             = "POS frontend"
  enabled             = true
